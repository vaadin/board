...
You can then import Vaadin Board inside your [elementname]`html` element (in `src/my-app/my-app.html`):
...
Insert the following code in your [elementname]`html` element (again, in `src/my-app/my-app.html`), inside [elementname]`template` tag:
